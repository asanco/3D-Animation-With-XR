...
namespace UnityEngine.UI
{
    ...
    public abstract class LoopScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        [SerializeField]
		//private typeScrollItem m_ItemType = typeScrollItem.Rumba;
		private typeScrollItem m_ItemType = typeScrollItem.Conciertos;
		public typeScrollItem ItemType { get { return m_ItemType; } set { m_ItemType = value; } }
...
//       		go.SendMessage("ScrollCellIndex", idx);
            ActionScrollItem (go, m_ItemType, idx);

            //LLAMO EL METODO QUE EJECUTA LA ACCION;
        ...
        public LoopVerticalScrollRect scrollDetect;

        void ActionScrollItem (Transform gm, typeScrollItem typeItem, int idx) 
		{

            switch (typeItem)  {
				/*case typeScrollItem.Rumba:
						gm.GetComponent<RumbaItemEvent>( ).setRumbaEvent( idx, scrollDetect );
                   		break;*/
			case typeScrollItem.Conciertos:
				//print ("---------------" + idx);
				/*if (gm.GetComponent<ConciertosItemEvent> () != null) 
				{
					gm.GetComponent<ConciertosItemEvent> ().setConciertoEvent (idx, scrollDetect);
				}*/
	            break;
				/*case typeScrollItem.Establecimientos:
						gm.GetComponent<EstablecimientosItemEvent>( ).setEstblEvent( idx , scrollDetect );
	                    break;
				case typeScrollItem.Reservas:
						gm.GetComponent<ReservaDataItem>( ).setReservaItem(idx, scrollDetect );
	                    break;
				case typeScrollItem.Promociones:
						gm.GetComponent<PromocionesItemEvent>( ).setPromocionEvent( idx , scrollDetect );
						break;*/
            }
        }

        public void ActionImageDowload (int idx)
        {
            if (scrollDetect.velocity.y == 0)
            { }
        }



		public void softScrollMove( int idx, int newPos, int finalPos){
			if((idx >= 0 && idx <=2)){
				scrollDetect.content.GetComponent<VerticalLayoutGroup>().padding = new RectOffset (0, 0, 0, 0);
			}else if((idx <= scrollDetect.totalCount-1 && idx >= scrollDetect.totalCount-3)){
				scrollDetect.content.GetComponent<VerticalLayoutGroup>().padding = new RectOffset (0, 0, finalPos, finalPos);
			}else{
				scrollDetect.content.GetComponent<VerticalLayoutGroup>().padding = new RectOffset (0, 0, newPos, newPos);
			}
		}
